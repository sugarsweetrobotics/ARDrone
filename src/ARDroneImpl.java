...
public class ARDroneImpl extends DataFlowComponentBase {
	...
	public ARDroneImpl(Manager manager) {
		...
		m_targetVelocityIn = new InPort<TimedVelocity3D>("targetVelocity",
				m_targetVelocity);
		m_camera_val = new CameraImage(new RTC.Time(0, 0), (short)0, (short)0, (short)0, "", 0., null);
		m_camera = new DataRef<CameraImage>(m_camera_val);
		m_cameraOut = new OutPort<CameraImage>("camera", m_camera);
		m_orientation_val = new TimedOrientation3D(new RTC.Time(0, 0), new Orientation3D(0., 0., 0.));
		m_orientation = new DataRef<TimedOrientation3D>(m_orientation_val);
		m_orientationOut = new OutPort<TimedOrientation3D>("orientation",
				m_orientation);
		m_attitude_val = new TimedDouble(new RTC.Time(0, 0), 0.);
		m_attitude = new DataRef<TimedDouble>(m_attitude_val);
		m_attitudeOut = new OutPort<TimedDouble>("attitude", m_attitude);
		m_currentVelocity_val = new TimedVelocity3D(new RTC.Time(0, 0), new Velocity3D(0., 0., 0., 0., 0., 0.));
		m_currentVelocity = new DataRef<TimedVelocity3D>(m_currentVelocity_val);
		m_currentVelocityOut = new OutPort<TimedVelocity3D>("currentVelocity",
				m_currentVelocity);
	...
	protected ReturnCode_t onInitialize() {
		...
		try {
			m_DroneServicePort.registerProvider("DroneService",
					"ssr.DroneService", m_droneService);
		...
		bindParameter("ipAddress", m_ipAddress, "192.168.1.1");
	...
	protected ReturnCode_t onActivated(int ec_id) {
		...
		drone.addImageUpdateListener(new ImageListener() {
			...
			public void imageUpdated(BufferedImage img) {
				imageBuffer = img;
				if (imageBuffer != null) {
					m_camera.v.width = (short) imageBuffer.getWidth();
					m_camera.v.height = (short) imageBuffer.getHeight();
					m_camera.v.pixels = new byte[m_camera.v.width * m_camera.v.height * 3];
					for (int h = 0;h < m_camera.v.height;h++) {
						for (int w = 0;w < m_camera.v.width;w++) {
							int index = h * m_camera.v.width + w;
							int rgb = imageBuffer.getRGB(w,  h);
							byte r = (byte)(0xFF & (rgb >> 16));
							byte g = (byte)(0xFF & (rgb >> 8));
							byte b = (byte)(0xFF & (rgb >> 0));
							m_camera.v.pixels[index*3 + 2] = r;
							m_camera.v.pixels[index*3 + 1] = g;
							m_camera.v.pixels[index*3 + 0] = b;
						}
					}
					m_cameraOut.write();
				}
	...
	protected ReturnCode_t onDeactivated(int ec_id) {
		...
		drone.removeAttitudeUpdateListener();
		drone.removeImageUpdateListener();
		drone.removeVelocityUpdateListener();
	...
	protected ReturnCode_t onExecute(int ec_id) {
		if (m_targetVelocityIn.isNew()) {
			...
			drone.move3D(
					(int) (this.m_velocityGainTranslation.value * m_targetVelocity.v.data.vx),
					(int) (this.m_velocityGainTranslation.value * m_targetVelocity.v.data.vy),
					(int) (this.m_velocityGainTranslation.value * m_targetVelocity.v.data.vz),
					(int) (this.m_velocityGainRotation.value * m_targetVelocity.v.data.va));
		}
